...
/* comment. Constructor(생성자) 주입. 권장 O */

@Service
public class PaymentServiceConstructor {
    // final = 불변 & 반드시 초기화(대입해주거나 생성자 호출 시점에 초기화)해줘야 함.
    // final 키워드의 특징 :불변성(상수)이다.
    // 따라서 final 키워드를 사용하려면 반드시 초기화를 필드에서 해주거나,
    // 생성자를 통해 초기화를 해줘야 함.
    private final PaymentInterface paymentInterface;

    /* comment.
    *   @Autowired 라는 어노테이션은 IoC 컨테이너에서 의존성이 필요한 경우에 자동으로 매개변수부에 있는 Bean을 찾아서 주입해준다.
    *   * 생성자 주입의 장점
    *   - final 키워드로 의존성이 변경되는 것을 방지한다. (불변성 보장)
    *   - 필수 의존성 강제화
    *    : final 키워드의 특징 초기화를 반드시 해야한다는 개념 때문에
    *      의존성을 객체가 생성되고 나중에 넣어주는 것이 아닌, 생성 시점에 바로 넣어준다.
    *   - 의존의 명확 : 코드만 보더라도, 해당 클래스가 어떤 클래스를 의존하는 지 알 수 있다.
    *   * 생성자 주입의 단점
    *   - 의존성이 많을 시에 생성자 매개변수 부분이 길어진다.
    *
    *   생성자 주입 방식은 Spring 환경에서 공식적으로 권장되는 방식이다.
    * */

    @Autowired  // 생성자가 1개이면 생략 가능. (but, 명시적 작성 권장)
    // public 생성자를 의미하는 구문
    public PaymentServiceConstructor(PaymentInterface paymentInterface){
        this.paymentInterface = paymentInterface;
    } // 생성자를 통한 의존성 주입.

    public boolean processPayment2(String orderId, double amount){
