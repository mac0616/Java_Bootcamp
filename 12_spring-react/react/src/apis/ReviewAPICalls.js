...
export const callReviewDetailAPI = ({ reviewCode }) => {
	const requestURL = `http://${import.meta.env.VITE_APP_RESTAPI_IP}:8080/api/v1/reviews/product/${reviewCode}`;
...
export const callReviewsAPI = ({ productCode, currentPage }) => {
	const requestURL = `http://${import.meta.env.VITE_APP_RESTAPI_IP}:8080/api/v1/reviews/${productCode}?offset=${currentPage}`;
