 ...
 * (422 에러). 우리는 application.yml의
 *   spring.jackson.property-naming-strategy: SNAKE_CASE
 * 한 줄로 해결했다. Boot가 자동 구성한 RestClient.Builder는 애플리케이션의
 * ObjectMapper를 그대로 쓰기 때문에 이 설정이 여기에도 적용된다.
 * → 그래서 builder를 '주입받아' 쓰는 것이 중요하다.
 *   RestClient.create()로 새로 만들면 이 설정을 잃어버린다!
 *
 * ─────────────────────────────────────────────────────────────────
...
public class RestClientConfig {
    ...
    public RestClient aiRestClient(RestClient.Builder builder) {
        ...
        return builder                       // ← Boot 자동구성 builder (snake_case 유지!)
