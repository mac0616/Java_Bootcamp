...
public class wanted_JDBCTemplate {
    ...
    *  */
    ...
    static {
        ...
        try {
            prop.load(wanted_JDBCTemplate.class.getClassLoader().getResourceAsStream("db-info.properties"));

            // HikariDataSource 를 구성 시 필요한 환경세팅 객체
            HikariConfig config = new HikariConfig();

            // db 정보 설정
            config.setJdbcUrl(prop.getProperty("db.wanted_url"));
            config.setUsername(prop.getProperty("db.wanted_username"));
            config.setPassword(prop.getProperty("db.wanted_password"));
